...
export default function AdminVendorsPage() {
  ...
  return (
    <div className="space-y-6">
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {vendor.status === 'PENDING' && (
                          <>
                            ...
                            <button
                              onClick={() => setPendingAction({ vendorId: vendor.id, vendorName: vendor.name, action: 'REJECTED' })}
                              disabled={processingId === vendor.id}
                        ...
                        {vendor.status === 'ACTIVE' && (
                          <button
                            onClick={() => setPendingAction({ vendorId: vendor.id, vendorName: vendor.name, action: 'SUSPENDED' })}
                            disabled={processingId === vendor.id}
                            className="px-3 py-1.5 rounded-lg bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/30 text-[11px] font-semibold flex items-center gap-1 transition-colors disabled:opacity-50 cursor-pointer"
                          >
      ...
      {/* Confirmation Modal for Suspend / Reject */}
      <ConfirmationModal
        isOpen={Boolean(pendingAction)}
        title={pendingAction?.action === 'SUSPENDED' ? 'Suspend Vendor Storefront' : 'Reject Vendor Application'}
        message={
          pendingAction?.action === 'SUSPENDED'
            ? `Are you sure you want to suspend "${pendingAction?.vendorName}"? All active product listings will be instantly hidden from customers and purchasing will be halted.`
            : `Are you sure you want to reject the application for "${pendingAction?.vendorName}"?`
        }
        confirmLabel={pendingAction?.action === 'SUSPENDED' ? 'Suspend Vendor' : 'Reject Application'}
        variant="danger"
        isLoading={Boolean(processingId)}
        onConfirm={() => {
          if (pendingAction) {
            handleStatusChange(pendingAction.vendorId, pendingAction.vendorName, pendingAction.action);
          }
        }}
