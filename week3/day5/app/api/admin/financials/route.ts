...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const record of allCommissionRecords) {
      ...
      const vendorEarning = Number(record.vendorEarning);
      if (record.status !== 'CANCELLED' && record.status !== 'REFUNDED') {
        totalMarketplaceSales = roundCurrency(totalMarketplaceSales + gross);
        totalPlatformCommission = roundCurrency(totalPlatformCommission + commission);
        totalVendorEarnings = roundCurrency(totalVendorEarnings + vendorEarning);
      } else if (record.status === 'REFUNDED') {
        refundAmount = roundCurrency(refundAmount + gross);
      }
    }
    ...
    for (const settlement of allSettlements) {
      const settAmount = Number(settlement.amount);
      if (settlement.status === 'PENDING' || settlement.status === 'PROCESSING') {
        pendingSettlements = roundCurrency(pendingSettlements + settAmount);
      } else if (settlement.status === 'PAID') {
        completedSettlements = roundCurrency(completedSettlements + settAmount);
      }
    }
