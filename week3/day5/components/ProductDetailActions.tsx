    ...
    <div className="space-y-4 pt-2">
      ...
      {!isOutOfStock && (
        <div className="flex items-center gap-3">
          <span className="text-[13px] text-(--text-on-dark-muted)">Quantity:</span>
          <div className="flex items-center border border-(--border-dark) bg-(--bg-header) rounded-lg overflow-hidden">
            <button
              type="button"
              onClick={() => setQuantity((q) => Math.max(1, q - 1))}
              className="p-2 text-(--text-on-dark-muted) hover:text-(--text-on-dark) hover:bg-(--bg-canvas) transition-colors cursor-pointer disabled:opacity-40"
              disabled={quantity <= 1}
            >
              <Minus className="w-3.5 h-3.5" />
            </button>
            <span className="px-3.5 text-xs font-bold text-(--text-on-dark) min-w-[32px] text-center">
              {quantity}
            </span>
            <button
              type="button"
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={`p-3 rounded-lg border transition-colors cursor-pointer flex items-center justify-center ${
            favorited
              ? 'bg-(--bg-header) border-[#1E7A56] text-[#1E7A56]'
              : 'bg-(--bg-header) border-(--border-dark) text-(--text-on-dark-muted) hover:text-(--text-on-dark) hover:border-[#1E7A56]'
          }`}
        >
