 ...
 */
export function roundCurrency(amount: number | { toNumber?: () => number } | string): number {
  const numericVal = typeof amount === 'object' && amount !== null && 'toNumber' in amount && typeof amount.toNumber === 'function'
    ? amount.toNumber()
    : Number(amount);
...
export async function getCurrentCommissionRate(client: any = prisma): Promise<number> {
  ...
  return latestSetting ? Number(latestSetting.rate) : 0.10;
}
 ...
 */
export function calculateCommission(grossAmount: number | { toNumber?: () => number } | string, rate: number | { toNumber?: () => number } | string) {
  const numGross = typeof grossAmount === 'object' && grossAmount !== null && 'toNumber' in grossAmount && typeof grossAmount.toNumber === 'function'
    ? grossAmount.toNumber()
    : Number(grossAmount);
  const numRate = typeof rate === 'object' && rate !== null && 'toNumber' in rate && typeof rate.toNumber === 'function'
    ? rate.toNumber()
    : Number(rate);

  const normalizedGross = roundCurrency(numGross);
  const commissionAmount = roundCurrency(normalizedGross * numRate);
...
export async function getVendorEarningsSummary(vendorId: string, client: any = prisma) {
  ...
  for (const record of commissionRecords) {
    ...
    if (record.status !== 'CANCELLED' && record.status !== 'REFUNDED') {
      const gross = Number(record.grossAmount);
      const commission = Number(record.commissionAmount);
      const earning = Number(record.vendorEarning);

      totalSales = roundCurrency(totalSales + gross);
      platformCommission = roundCurrency(platformCommission + commission);
      netEarnings = roundCurrency(netEarnings + earning);

      if (record.status === 'PENDING') {
        pendingEarnings = roundCurrency(pendingEarnings + earning);
      } else if (record.status === 'EARNED' && !record.settlementItem) {
        // Available Balance = EARNED with no SettlementItem attached
        availableBalance = roundCurrency(availableBalance + earning);
      }
    }
  }
