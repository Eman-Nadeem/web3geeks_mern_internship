...
export function errorResponse(message: string, statusCode: number = 400, details?: unknown) {
  // Sanitize internal errors in production to prevent leaking database or server internals
...
/**
 * Strictly verifies that a Payment belongs to the specified customer (or allows ADMIN).
 */
export async function assertPaymentOwnership(paymentId: string, customerId: string, isAdmin: boolean = false) {
  ...
  if (!isAdmin && payment.customerId !== customerId) {
    throw new AppError('Forbidden: You do not own this payment record.', 403);
  }
...
/**
 * Strictly verifies that a Settlement belongs to the specified vendor.
 */
export async function assertSettlementOwnership(settlementId: string, vendorId: string) {
  ...
  if (settlement.vendorId !== vendorId) {
    throw new AppError('Forbidden: You do not own this settlement.', 403);
  }
 ...
 * PROCESSING -> REJECTED
 */
export const ALLOWED_SETTLEMENT_TRANSITIONS: Record<string, string[]> = {
...
export function isValidSettlementTransition(currentStatus: string, nextStatus: string): boolean {
  if (currentStatus === nextStatus) return true;
