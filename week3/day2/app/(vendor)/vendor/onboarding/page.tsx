  ...
  return (
    <div className="max-w-2xl mx-auto py-10 space-y-8">
      <div className="text-center space-y-2">
        <div className="w-14 h-14 rounded-2xl bg-emerald-600 flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/20">
          <Store className="w-7 h-7 text-white" />
        </div>
        <h1 className="text-3xl font-extrabold text-[var(--text-on-dark)] tracking-tight">Vendor Store Registration</h1>
        <p className="text-xs text-[var(--text-on-dark-muted)] max-w-md mx-auto">
      ...
      <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 flex items-start gap-3 text-xs text-emerald-400">
        <ShieldCheck className="w-4 h-4 shrink-0 mt-0.5 text-emerald-500" />
        <div>
          <span className="font-semibold text-[var(--text-on-dark)]">Approval Workflow:</span> Your store will be registered in <span className="font-bold text-amber-400">PENDING</span> status. After marketplace approval, your public storefront and products will immediately become visible to shoppers worldwide.
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 rounded-3xl bg-[var(--surface-card)] border border-[var(--border-dark)] space-y-5 shadow-2xl">
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-[var(--text-on-dark-muted)]">Store Slug (URL Identifier) *</label>
          <div className="relative">
            ...
            <input
              ...
              placeholder="e.g. apex-hardware-labs"
              className="w-full pl-9 pr-3 py-2 bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] rounded-xl text-xs text-emerald-400 font-mono placeholder-[var(--text-on-dark-muted)] focus:outline-none focus:border-emerald-500"
            />
          </div>
          <span className="text-[11px] text-[var(--text-on-dark-muted)] block">
            Public storefront URL: <span className="text-emerald-500">/vendors/{slug || 'your-slug'}</span>
          </span>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-xs transition-all shadow-lg shadow-emerald-600/20 disabled:opacity-50 flex items-center justify-center gap-2 mt-4 cursor-pointer"
        >
