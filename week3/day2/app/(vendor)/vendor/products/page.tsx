  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[var(--text-on-dark)] tracking-tight">Product Catalog</h2>
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="px-4 py-2 rounded-xl bg-[var(--surface-card-subtle)] hover:bg-[var(--border-dark)] text-[var(--text-on-dark)] font-semibold text-xs flex items-center justify-center gap-2 border border-[var(--border-dark)] transition-colors"
          >
      ...
      <div className="p-4 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-dark)] space-y-3 shadow-sm">
        <form onSubmit={handleSearchSubmit} className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] rounded-xl text-xs text-[var(--text-on-dark)] placeholder:text-[var(--text-on-dark-muted)] focus:outline-none focus:border-emerald-500"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] rounded-xl text-xs text-[var(--text-on-dark)] focus:outline-none focus:border-emerald-500"
          >
        ...
        {/* Status Tabs */}
        <div className="flex items-center gap-2 pt-2 border-t border-[var(--border-dark)] overflow-x-auto">
      ...
      {loading ? (
        ...
        <div className="py-16 rounded-2xl bg-[var(--surface-card)] border border-[var(--border-dark)] text-center space-y-3">
          ...
          <h3 className="text-sm font-semibold text-[var(--text-on-dark)]">No products match your criteria</h3>
          <p className="text-xs text-[var(--text-on-dark-muted)]">Add a new item to your catalog or clear filter options.</p>
        </div>
      ) : (
                ...
                {products.map((product) => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-[var(--surface-card-subtle)] transition-colors">
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-xl bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] overflow-hidden flex items-center justify-center shrink-0">
                            {primaryImg ? (
                              <img src={primaryImg.url} alt={product.name} className="w-full h-full object-cover" />
                            ) : (
                              <Box className="w-5 h-5 text-[var(--text-on-dark-muted)]" />
                            )}
                          </div>
                          <div>
                            <div className="font-bold text-[var(--text-on-dark)] text-xs">{product.name}</div>
                            <span className="text-[11px] text-[var(--text-on-dark-muted)] font-mono">slug: {product.slug}</span>
                      ...
                      <td className="p-4 text-[var(--text-on-dark-muted)] font-medium">{product.category}</td>
                      <td className="p-4">
                        <div className="font-bold text-emerald-600 dark:text-emerald-400">${product.price.toFixed(2)}</div>
                        {product.compareAtPrice && (
                          <div className="text-[11px] text-[var(--text-on-dark-muted)] line-through">
                          ...
                          <span
                            className={`font-semibold px-2 py-0.5 rounded text-[11px] ${
                              isOut
                                ...
                                : 'bg-[var(--surface-card-subtle)] text-[var(--text-on-dark)]'
                            }`}
                          >
                            {product.stockQuantity} units
                          </span>
                        </div>
                        <span className="text-[10px] text-[var(--text-on-dark-muted)] block mt-0.5">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-cyan-600 dark:text-cyan-400 hover:bg-[var(--surface-card-subtle)] transition-colors cursor-pointer"
                          ...
                          <Link
                            ...
                            className="p-1.5 rounded-lg text-[var(--text-on-dark-muted)] hover:text-[var(--text-on-dark)] hover:bg-[var(--surface-card-subtle)] transition-colors inline-block"
                            title="Edit Product"
                          >
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg text-[var(--text-on-dark-muted)] hover:text-rose-500 hover:bg-rose-500/10 transition-colors disabled:opacity-30 cursor-pointer"
      ...
      {adjustingProduct && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[var(--surface-card)] border border-[var(--border-dark)] rounded-3xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-[var(--border-dark)] pb-3">
              <div>
                <h3 className="text-sm font-bold text-[var(--text-on-dark)]">Adjust Stock Level</h3>
                <p className="text-xs text-[var(--text-on-dark-muted)] truncate max-w-[280px]">
            ...
            <form onSubmit={handleStockAdjustSubmit} className="space-y-4 text-xs">
              ...
              {adjustingProduct.variants && adjustingProduct.variants.length > 0 && (
                <div className="space-y-1.5">
                  <label className="text-[var(--text-on-dark)] font-semibold">Select Variant</label>
                  <select
                    value={selectedVariantId}
                    onChange={(e) => setSelectedVariantId(e.target.value)}
                    className="w-full px-3 py-2 bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] rounded-xl text-[var(--text-on-dark)] focus:outline-none focus:border-emerald-500"
                  >
              ...
              <div className="p-3 rounded-xl bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] flex items-center justify-between">
                <span className="text-[var(--text-on-dark-muted)]">Current Stock:</span>
                <span className="font-bold text-[var(--text-on-dark)] text-sm">
                  {selectedVariantId
              ...
              <div className="space-y-1.5">
                <label className="text-[var(--text-on-dark)] font-semibold">Adjustment Delta</label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                  ...
                  <input
                    ...
                    className="w-20 px-3 py-1.5 bg-[var(--surface-card-subtle)] border border-[var(--border-dark)] rounded-lg text-center font-bold text-[var(--text-on-dark)] text-sm focus:outline-none focus:border-emerald-500"
                  />
                  <button
                    type="button"
                    onClick={() => setAdjustDelta((d) => d + 1)}
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-[var(--border-dark)]">
                <button
                  type="button"
                  onClick={() => setAdjustingProduct(null)}
                  className="px-4 py-2 rounded-xl bg-[var(--surface-card-subtle)] hover:bg-[var(--border-dark)] text-[var(--text-on-dark)] font-semibold cursor-pointer border border-[var(--border-dark)]"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-[var(--surface-card)] border border-[var(--border-dark)] rounded-3xl p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 pr-1">
              {historyLoading ? (
                <div className="py-12 text-center text-xs text-[var(--text-on-dark-muted)]">Loading history logs...</div>
