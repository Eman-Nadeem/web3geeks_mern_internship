...
export const createPaymentSchema = z.object({
  ...
  method: z.enum(['COD', 'MOCK_GATEWAY']).default('MOCK_GATEWAY'),
});

export const verifyPaymentSchema = z.object({
  referenceId: z.string().trim().optional(),
  simulatedStatus: z.enum(['SUCCESS', 'FAILED']).optional(),
});
...
export const createSettlementSchema = z.object({
  amount: z.coerce.number().positive('Settlement amount must be positive').optional(),
  periodStart: z.coerce.date().optional(),
  periodEnd: z.coerce.date().optional(),
});

export const updateSettlementStatusSchema = z
  .object({
    status: z.enum(['PENDING', 'PROCESSING', 'PAID', 'REJECTED'], {
      message: 'Status must be PENDING, PROCESSING, PAID, or REJECTED',
    }),
    paymentReference: z.string().trim().optional().nullable(),
  })
  .refine((data) => (data.status !== 'PAID' ? true : Boolean(data.paymentReference && data.paymentReference.trim().length > 0)), {
    message: 'Payment reference is strictly required when marking a settlement as PAID',
...
export const financialFilterSchema = z.object({
  ...
  type: z.enum(['PAYMENT', 'SALE', 'COMMISSION', 'REFUND', 'SETTLEMENT', 'ADJUSTMENT']).optional(),
...
export type CommissionSettingInput = z.infer<typeof commissionSettingSchema>;
