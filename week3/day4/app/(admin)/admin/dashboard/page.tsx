...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-(--text-on-dark) tracking-tight">System Status & Metrics</h2>
        <p className="text-xs text-(--text-on-dark-muted) mt-1">Platform-wide statistics and pending operational tasks</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="p-5 rounded-2xl bg-(--surface-card) border border-(--border-dark) shadow-xs space-y-2">
        ...
        <div className="p-5 rounded-2xl bg-(--surface-card) border border-(--border-dark) shadow-xs space-y-2">
          ...
          <div className="text-3xl font-extrabold text-(--text-on-dark)">{totalVendors}</div>
          <span className="text-[11px] text-(--text-on-dark-muted) block">Registered stores</span>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-(--surface-card) border border-(--border-dark) shadow-xs space-y-2">
          ...
          <div className="text-3xl font-extrabold text-(--text-on-dark)">{totalProducts}</div>
          <span className="text-[11px] text-(--text-on-dark-muted) block">Multi-vendor catalog items</span>
        </div>
        ...
        <div className="p-5 rounded-2xl bg-(--surface-card) border border-(--border-dark) shadow-xs space-y-2">
          ...
          <div className="text-3xl font-extrabold text-(--text-on-dark)">{totalUsers}</div>
          <span className="text-[11px] text-(--text-on-dark-muted) block">Customers & merchant owners</span>
        </div>
      </div>

      <div className="p-6 rounded-2xl bg-(--surface-card) border border-(--border-dark) shadow-xs flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="space-y-1 text-center sm:text-left">
          <h3 className="text-base font-bold text-(--text-on-dark)">Vendor Review & Approval Queue</h3>
