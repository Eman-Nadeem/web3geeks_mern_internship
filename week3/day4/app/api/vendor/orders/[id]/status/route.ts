...
export async function PATCH(req: NextRequest, context: RouteContext) {
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      // Financial lifecycle transitions
      if (nextStatus === 'DELIVERED') {
        if (tx.commissionRecord) {
          // Transition CommissionRecord status from PENDING to EARNED
        ...
        const commRecord = updatedVendorOrder.commissionRecord;
          ...
          if (tx.financialTransaction) {
            ...
            await tx.financialTransaction.create({
              data: {
                ...
                direction: 'DEBIT',
                description: `Cancellation reversal of gross sale for VendorOrder ${vendorOrder.vendorOrderNumber}`,
              },
            });
            ...
            await tx.financialTransaction.create({
              data: {
                ...
                direction: 'CREDIT',
                description: `Cancellation reversal of platform commission for VendorOrder ${vendorOrder.vendorOrderNumber}`,
              },
            });

            if (commRecord.settlementItem) {
              // Already paid out to vendor: create negative recovery adjustment entry
              await tx.financialTransaction.create({
                data: {
                  vendorId: vendor.id,
                  orderId: vendorOrder.orderId,
                  vendorOrderId: vendorOrder.id,
                  type: 'ADJUSTMENT',
                  amount: commRecord.vendorEarning,
                  direction: 'DEBIT',
                  description: `Post-settlement cancellation adjustment to be recovered for VendorOrder ${vendorOrder.vendorOrderNumber}`,
                },
              });
            }
          }
