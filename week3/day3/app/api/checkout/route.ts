...
export async function POST(req: NextRequest) {
  try {
    ...
    // Run the entire multi-vendor order creation inside an atomic transaction
    const completedOrder = await prisma.$transaction(async (tx) => {
      ...
      for (const item of cart.items) {
        ...
        let liveUnitPrice = product.price;
        let sku = product.sku;
        ...
        if (item.variantId) {
          ...
          if (selectedVariant.stockQuantity < item.quantity) {
            throw new AppError(`Insufficient stock for "${product.name}" (variant SKU: ${selectedVariant.sku}). Available: ${selectedVariant.stockQuantity}, Requested: ${item.quantity}.`, 400);
          }
          if (selectedVariant.price != null) {
            liveUnitPrice = selectedVariant.price;
          }
      ...
      const parentOrder = await tx.order.create({
        data: {
          ...
          paymentStatus: 'PAID', // Simulated immediate payment confirmation for Day 3
          shippingName,
          shippingEmail,
          shippingPhone,
          shippingAddress,
