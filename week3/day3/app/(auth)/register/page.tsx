  ...
  return (
    <div className="w-full flex flex-col justify-center items-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-md text-center space-y-2 mb-6">
        ...
        <h1 className="text-2xl font-bold text-(--text-on-dark) tracking-tight">Create your account</h1>
        <p className="text-[13px] text-(--text-on-dark-muted)">Join Nexus Market as a shopper or verified seller</p>
      </div>
      ...
      <div className="w-full max-w-md bg-(--surface-card) rounded-2xl border border-(--border-dark) shadow-xl p-6 sm:p-8 text-(--text-on-dark)">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <div>
            <label className="block text-[13px] font-medium text-(--text-on-dark) mb-1.5">
              I want to:
            </label>
              ...
              <button
                ...
                className={`py-2 px-3 rounded-xl border text-[13px] font-semibold flex items-center justify-center gap-1.5 transition-colors cursor-pointer ${
                  role === 'CUSTOMER'
                    ...
                    : 'border-(--border-dark) text-(--text-on-dark-muted) bg-(--surface-card-subtle) hover:bg-(--border-dark)'
                }`}
              >
          ...
          <div>
            ...
            <div className="relative">
              <User className="w-4 h-4 text-(--text-on-dark-muted) absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" />
              <input
                type="text"
                required
                value={name}
