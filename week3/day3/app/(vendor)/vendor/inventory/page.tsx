    ...
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2">
            <Link
              ...
              className="text-xs font-semibold text-(--text-on-dark-muted) hover:text-(--text-on-dark) flex items-center gap-1 transition-colors"
            >
              <ArrowLeft className="w-3.5 h-3.5" />
              Products
            </Link>
          </div>
          <h2 className="text-2xl font-bold text-(--text-on-dark) tracking-tight mt-1">Inventory Management</h2>
      ...
      {stats && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 rounded-2xl bg-(--surface-card) border border-(--border-dark) space-y-1 shadow-sm">
            ...
            <span className="text-[11px] text-(--text-on-dark-muted) block">Across all catalog products</span>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-(--surface-card) border border-(--border-dark) space-y-1 shadow-sm">
            ...
            <span className="text-[11px] text-(--text-on-dark-muted) block">At or below product threshold</span>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-(--surface-card) border border-(--border-dark) space-y-1 shadow-sm">
            ...
            <span className="text-[11px] text-(--text-on-dark-muted) block">Requires restocking</span>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-(--surface-card) border border-(--border-dark) space-y-1 shadow-sm">
            ...
            <span className="text-[11px] text-(--text-on-dark-muted) block">Published live in store</span>
          </div>
        </div>
      )}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 bg-(--surface-card-subtle) border border-(--border-dark) rounded-xl text-xs text-(--text-on-dark) placeholder:text-(--text-on-dark-muted) focus:outline-none focus:border-emerald-500"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="py-16 rounded-2xl bg-(--surface-card) border border-(--border-dark) text-center space-y-3">
          ...
          <h3 className="text-sm font-semibold text-(--text-on-dark)">No inventory matches</h3>
          <p className="text-xs text-(--text-on-dark-muted)">All products are healthy or match your current filter.</p>
        </div>
      ) : (
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    <React.Fragment key={product.id}>
                      <tr className="hover:bg-(--surface-card-subtle) transition-colors">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-xl bg-(--surface-card-subtle) border border-(--border-dark) overflow-hidden flex items-center justify-center shrink-0">
                              {primaryImg ? (
                            ...
                            <div>
                              ...
                              <span className="text-[11px] text-(--text-on-dark-muted)">{product.category}</span>
                              {hasVariants && (
                        ...
                        <td className="p-4 text-(--text-on-dark-muted)">{product.lowStockThreshold} units</td>
                        <td className="p-4">
                          {!hasVariants ? (
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                className="px-2 py-1 bg-(--surface-card-subtle) hover:bg-(--border-dark) border border-(--border-dark) rounded-lg text-(--text-on-dark) font-bold text-[11px] disabled:opacity-40 cursor-pointer"
                              >
                                -5
                              </button>
                              <button
                            ...
                            <span className="text-[11px] text-(--text-on-dark-muted) italic">Adjust below per variant</span>
                          )}
                        </td>
                        <td className="p-4">
                          {!hasVariants && (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-(--surface-card-subtle) hover:bg-(--border-dark) text-(--text-on-dark) border border-(--border-dark) text-xs font-semibold transition-colors cursor-pointer"
                            >
                              Set Exact
                            </button>
                          )}
                      ...
                      {hasVariants &&
                        product.variants.map((variant) => {
                          ...
                          return (
                            <tr key={variant.id} className="bg-(--surface-card-subtle)/50 hover:bg-(--surface-card-subtle) transition-colors">
                              <td className="py-2.5 pl-12 pr-4 text-(--text-on-dark)">
                                <div className="flex items-center gap-2">
                              ...
                              <td className="py-2.5 px-4 text-(--text-on-dark-muted) text-[11px]">-</td>
                              <td className="py-2.5 px-4">
                                <div className="flex items-center gap-1.5">
                                  <button
                                    type="button"
                                    ...
                                    className="px-2 py-0.5 bg-(--surface-card-subtle) hover:bg-(--border-dark) border border-(--border-dark) rounded text-(--text-on-dark) font-bold text-[10px] disabled:opacity-40 cursor-pointer"
                                  >
                                    -1
                                  </button>
                                  <button
                              ...
                              <td className="py-2.5 px-4">
                                <button
                                  ...
                                  className="px-2 py-0.5 rounded bg-(--surface-card-subtle) hover:bg-(--border-dark) text-(--text-on-dark) border border-(--border-dark) text-[11px] font-semibold cursor-pointer"
                                >
                                  Set
                                </button>
                              </td>
                              <td className="py-2.5 px-4 text-right">
                                <span className="text-[10px] text-(--text-on-dark-muted)">variant level</span>
                              </td>
                            </tr>
                          );
                        })}
      ...
      {setStockModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-(--surface-card) border border-(--border-dark) rounded-3xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-(--border-dark) pb-3">
              <h3 className="text-sm font-bold text-(--text-on-dark)">Set Exact Inventory Count</h3>
              <button onClick={() => setSetStockModal(null)} className="text-(--text-on-dark-muted) hover:text-(--text-on-dark) cursor-pointer">
                <X className="w-4 h-4" />
              </button>
            </div>

            <form onSubmit={handleExactSetSubmit} className="space-y-4 text-xs">
              <div className="p-3 rounded-xl bg-(--surface-card-subtle) border border-(--border-dark)">
                <div className="text-(--text-on-dark-muted) text-[11px]">Product:</div>
                <div className="font-bold text-(--text-on-dark) truncate">{setStockModal.product.name}</div>
                <div className="text-(--text-on-dark-muted) text-[11px] mt-1">Current Stock: {setStockModal.currentStock} units</div>
              </div>

              <div className="space-y-1.5">
                <label className="text-(--text-on-dark) font-semibold">New Exact Quantity *</label>
                <input
                  type="number"
                  min="0"
                  required
                  ...
                  className="w-full px-3 py-2 bg-(--surface-card-subtle) border border-(--border-dark) rounded-xl text-(--text-on-dark) font-bold text-base focus:outline-none focus:border-emerald-500"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-3 border-t border-(--border-dark)">
                <button
                  ...
                  className="px-4 py-2 rounded-xl bg-(--surface-card-subtle) hover:bg-(--border-dark) text-(--text-on-dark) border border-(--border-dark) font-semibold cursor-pointer"
                >
                  Cancel
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-(--surface-card) border border-(--border-dark) rounded-3xl p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
            ...
            <div className="overflow-y-auto flex-1 pr-1">
              {historyLoading ? (
                <div className="py-12 text-center text-xs text-(--text-on-dark-muted)">Loading audit history...</div>
