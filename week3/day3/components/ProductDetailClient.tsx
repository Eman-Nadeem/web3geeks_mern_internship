...
export function ProductDetailClient({ product }: ProductDetailProps) {
  ...
  return (
    <div className="w-full min-h-[85vh] bg-(--bg-canvas) text-(--text-on-dark) py-8 pb-16 transition-colors duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        {/* Navigation Breadcrumb */}
        <div className="flex items-center gap-2 text-[13px] text-(--text-on-dark-muted)">
          <Link
            href="/products"
            className="inline-flex items-center gap-1.5 hover:text-(--text-on-dark) transition-colors"
          >
          ...
          <span>/</span>
          <span className="text-(--text-on-dark) font-medium truncate max-w-50 sm:max-w-md">
            {product.name}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-4">
            <div className="rounded-2xl bg-white border border-[#E5E7EB] p-8 shadow-sm flex flex-col items-center justify-center relative min-h-95 sm:min-h-110 overflow-hidden">
              {displayImage ? (
                <img
                  src={displayImage}
                  alt={product.name}
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-(--text-on-dark-muted)">Sold by:</span>
              <Link
                href={`/vendors/${product.vendor.slug}`}
                className="text-xs font-semibold text-(--text-on-dark) hover:text-[#1E7A56] flex items-center gap-1.5 transition-colors bg-(--bg-header) px-2.5 py-1 rounded border border-(--border-dark)"
              >
            ...
            {/* Title & SKU */}
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-(--text-on-dark) tracking-tight leading-snug">
                {product.name}
              </h1>
              <div className="flex items-center gap-3 text-xs text-(--text-on-dark-muted) mt-1 font-mono">
            ...
            <div className="flex items-center gap-3 text-xs text-(--text-on-dark-muted)">
              ...
              <span className="font-semibold text-(--text-on-dark)">4.8</span>
              <span>•</span>
            ...
            {/* Price & Stock status */}
            <div className="flex items-baseline gap-4 pt-2 border-t border-(--border-dark)">
              <span className="text-3xl font-bold text-(--text-on-dark)">
                {formattedPrice}
              </span>
              {formattedComparePrice && (
                <span className="text-lg text-(--text-on-dark-muted) line-through">
                  {formattedComparePrice}
                </span>
              )}
              <div className="flex items-center gap-1.5 text-xs text-(--text-on-dark-muted) ml-auto sm:ml-0">
                {isOutOfStock ? (
            ...
            {product.variants.length > 0 && optionKeys.length > 0 && (
              <div className="p-4 rounded-xl bg-(--surface-card) border border-(--border-dark) space-y-4">
                <div className="flex items-center gap-2 text-xs font-semibold text-(--text-on-dark) uppercase tracking-wider">
                ...
                {optionKeys.map((key) => {
                  ...
                  return (
                    <div key={key} className="space-y-1.5">
                      <label className="text-xs text-(--text-on-dark-muted) font-medium">
                        {key}: <span className="font-bold text-(--text-on-dark)">{selectedOptions[key] || 'Select'}</span>
                      </label>
                            ...
                            <button
                              ...
                              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer ${
                                isSelected
                                  ...
                                  : 'bg-(--surface-card-subtle) text-(--text-on-dark) hover:border-emerald-500 border border-(--border-dark)'
                              }`}
                            >
                              {val}
                            </button>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button
                ...
                className="p-3.5 rounded-xl bg-(--surface-card-subtle) hover:bg-(--surface-card) border border-(--border-dark) text-(--text-on-dark) transition-colors cursor-pointer"
              >
                <Heart
                  className={`w-5 h-5 ${
                    favorited ? 'fill-[#1E7A56] text-[#1E7A56]' : 'text-(--text-on-dark-muted)'
                  }`}
                />
              </button>
            </div>
            ...
            <div className="pt-4 border-t border-(--border-dark) space-y-2">
              <h2 className="text-xs font-semibold text-(--text-on-dark) uppercase tracking-wider">
                Product Details
              </h2>
              <p className="text-[14px] text-(--text-on-dark-muted) leading-relaxed whitespace-pre-line">
                {product.description}
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className="p-3 rounded-lg bg-(--bg-header) border border-(--border-dark) flex items-center gap-2.5">
                <Truck className="w-4 h-4 text-[#1E7A56] shrink-0" />
                <div className="text-xs leading-tight">
                  <div className="font-semibold text-(--text-on-dark)">Free Standard Delivery</div>
                  <div className="text-[11px] text-(--text-on-dark-muted)">Direct from verified seller</div>
                </div>
              </div>
              <div className="p-3 rounded-lg bg-(--bg-header) border border-(--border-dark) flex items-center gap-2.5">
                <ShieldCheck className="w-4 h-4 text-[#1E7A56] shrink-0" />
                <div className="text-xs leading-tight">
                  <div className="font-semibold text-(--text-on-dark)">Marketplace Guarantee</div>
                  <div className="text-[11px] text-(--text-on-dark-muted)">Direct seller warranty</div>
                </div>
              </div>
            </div>

            {/* Owning Vendor Info Card (SAFE: strictly NO vendor email/phone) */}
            <div className="rounded-xl bg-(--surface-card) p-5 border border-(--border-dark) text-(--text-on-dark) space-y-3 shadow-xs">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-(--bg-header) text-(--text-on-dark) border border-(--border-dark) flex items-center justify-center font-bold text-sm shrink-0">
              ...
              {product.vendor.description && (
                <p className="text-[12px] text-(--text-on-dark-muted) line-clamp-2">
                  {product.vendor.description}
                </p>
              )}

              <div className="pt-2 border-t border-(--border-dark) flex items-center justify-between text-xs">
                <span className="text-(--text-on-dark-muted)">Independent Seller Catalog</span>
                <Link
                  href={`/vendors/${product.vendor.slug}`}
