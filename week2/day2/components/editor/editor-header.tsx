...
import { Collaborator } from "@/lib/realtime/events";
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
          ...
          <div className="flex items-center -space-x-1.5 overflow-hidden">
            {activeUsers.length === 0 && user && (
              <div
                className="w-7 h-7 rounded-full border-2 border-white bg-(--accent-primary) text-white text-[11px] font-bold flex items-center justify-center overflow-hidden shadow-xs"
                title={`${user.name} (You)`}
              >
                {user.avatarUrl ? (
            ...
            {activeUsers.map((collaborator) => {
              ...
              return (
                <div
                  key={collaborator.id}
                  className="w-7 h-7 rounded-full border-2 border-white bg-(--accent-soft-bg) text-(--accent-primary) text-[11px] font-bold flex items-center justify-center overflow-hidden shadow-xs transition-transform hover:scale-110 hover:z-10"
