...
export function EditorClientContainer({
  ...
  const [currentAccessRole, setCurrentAccessRole] = useState<AccessRole>(accessRole);
  ...
  // Day 4: Real-time Permission Update (demoted to viewer or access revoked/booted)
      ...
      if (user && payload.userId === user.id) {
        if (payload.revoked) {
          ...
          addToast({
            ...
            message: payload.message || "Your collaborator access was removed by the owner.",
            duration: 8000,
          });
        } else if (payload.role) {
          setCurrentAccessRole(payload.role);
          addToast({
            type: "info",
            title: "Permissions Updated",
            message: `Your document role was updated to ${payload.role.toUpperCase()}`,
            duration: 5000,
          });
        }
      }
  ...
  // Day 4: Real-time Document Restored by peer
  const handleDocumentRestored = useCallback(
    (payload: DocumentRestoredPayload) => {
      ...
      addToast({
        ...
        message: `Restored to Version ${payload.restoredFromVersion} by ${payload.restoredBy.name}`,
        userName: payload.restoredBy.name,
        avatarUrl: payload.restoredBy.avatarUrl,
        duration: 6000,
      });
    },
    [addToast]
  ...
  // Day 4: Restore version action passed to EditorHeader / VersionHistoryModal
    ...
    async (versionNumber: number): Promise<boolean> => {
      try {
        ...
        if (res.ok) {
          ...
          const restoredDoc = json.data?.document;
          if (restoredDoc) {
  ...
  const handleContentChange = (newHtml: string, newJson?: string) => {
    ...
    if (connectionState !== "connected") {
      ...
      // 600ms debounce interval: Intentionally chosen for offline/disconnected HTTP autosave fallback,
      // buffering keystrokes to minimize expensive REST database writes while ensuring unsaved local edits are captured.
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      ...
      {/* Booted / Access Revoked Modal Overlay */}
      {isBooted && (
        <div className="fixed inset-0 z-70 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl text-center space-y-4 border border-slate-200">
            ...
            <p className="text-xs text-slate-600">
              The document owner has removed your collaborator permissions. You no longer have access to view or edit this document.
            </p>
            <button
              onClick={() => router.push("/documents")}
              className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl shadow-xs transition-colors cursor-pointer"
            >
              Return to Documents
            </button>
          </div>
