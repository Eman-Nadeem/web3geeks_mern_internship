...
export async function DELETE(_req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Trigger instant eviction from active WebSocket room session
    const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || "http://localhost:3001";
    try {
      await fetch(`${socketUrl}/api/socket/evict`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          documentId: id,
          targetUserId,
          reason: "Access revoked by document owner",
        }),
      }).catch(() => {});
    } catch {}
...
export async function PATCH(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Trigger instant role change broadcast across active room sessions
    const socketUrl = process.env.NEXT_PUBLIC_SOCKET_URL || "http://localhost:3001";
    try {
      await fetch(`${socketUrl}/api/socket/role-change`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-internal-bridge-secret": process.env.INTERNAL_BRIDGE_SECRET || "",
