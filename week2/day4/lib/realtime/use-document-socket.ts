...
interface UseDocumentSocketOptions {
  ...
  onPermissionUpdate?: (payload: PermissionUpdatePayload) => void;
  onCollaboratorAdded?: (payload: CollaboratorAddedPayload) => void;
...
export function useDocumentSocket({
  ...
  const onCollaboratorAddedRef = useRef(onCollaboratorAdded);
  ...
  useEffect(() => {
    ...
    onCollaboratorAddedRef.current = onCollaboratorAdded;
  ...
  useEffect(() => {
    ...
    async function initSocket() {
      ...
      // Day 4: Permission / Role Demotion / Revocation
      socket.on(REALTIME_EVENTS.PERMISSION_UPDATE, (data: PermissionUpdatePayload) => {
        if (data.documentId === documentId) {
          onPermissionUpdateRef.current?.(data);
        }
      });

      // Day 4: Live Collaborator Added
      socket.on(REALTIME_EVENTS.COLLABORATOR_ADDED, (data: CollaboratorAddedPayload) => {
        if (data.documentId === documentId) {
          onCollaboratorAddedRef.current?.(data);
        }
      });

      // Day 4: Live Collaborator Removed
      socket.on(REALTIME_EVENTS.COLLABORATOR_REMOVED, (data: CollaboratorRemovedPayload) => {
        if (data.documentId === documentId) {
          onCollaboratorRemovedRef.current?.(data);
        }
      });

      // Day 4: Live Role Changed
      socket.on(REALTIME_EVENTS.ROLE_CHANGED, (data: RoleChangedPayload) => {
        if (data.documentId === documentId) {
          onRoleChangedRef.current?.(data);
        }
      });

      // Day 4: Live Document Restored
      socket.on(REALTIME_EVENTS.DOCUMENT_RESTORED, (data: DocumentRestoredPayload) => {
        if (data.documentId === documentId) {
          if (data.version !== undefined) {
            documentVersionRef.current = data.version;
            setDocumentVersion(data.version);
  ...
  const sendChange = useCallback(
    (change: {
      ...
      // 150ms debounce interval: Intentionally chosen to throttle rapid keystrokes and prevent
      // socket frame flooding while providing near-instantaneous live sync to co-authors with zero perceptible lag.
  ...
  const changeRole = useCallback(
    (targetUserId: string, newRole: "editor" | "viewer") => {
      const socket = socketRef.current;
      if (socket && socket.connected) {
        socket.emit(REALTIME_EVENTS.ROLE_CHANGED, {
          documentId,
          userId: targetUserId,
          newRole,
        });
  ...
  const removeCollaborator = useCallback(
    (targetUserId: string) => {
      const socket = socketRef.current;
      if (socket && socket.connected) {
        socket.emit(REALTIME_EVENTS.COLLABORATOR_REMOVED, {
          documentId,
          userId: targetUserId,
        });
      }
  ...
  const restoreDocument = useCallback(
    (payload: DocumentRestoredPayload) => {
      const socket = socketRef.current;
      if (socket && socket.connected) {
        socket.emit(REALTIME_EVENTS.DOCUMENT_RESTORED, payload);
      }
    },
    []
  );
