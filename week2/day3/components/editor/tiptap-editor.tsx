...
import { CollaborationCursor, RemoteCursor } from "./collaboration-cursor";
import {
  Bold,
  Italic,
  Heading1,
...
interface TiptapEditorProps {
  ...
  onCursorChange?: (cursor: { from: number; to: number } | null) => void;
}
...
export function TiptapEditor({
  ...
  const editor = useEditor({
    ...
    onSelectionUpdate: ({ editor }) => {
      // Priority 4: Guard against echo loops from programmatic setContent or cursor decoration updates
  ...
  // Dynamically sync remote cursors into the ProseMirror plugin state
  useEffect(() => {
    if (editor && remoteCursors) {
  ...
  // External content sync (Priority 4: Preserves local selection and suppresses echo loop)
  useEffect(() => {
    if (!editor) return;
    ...
    if (normalized !== normalizedLastEmitted && normalized !== normalizedEditor) {
      ...
      // 2. Apply remote update
      editor.commands.setContent(normalized, { emitUpdate: false });

      // 3. Safely restore local selection clamped to new document bounds
