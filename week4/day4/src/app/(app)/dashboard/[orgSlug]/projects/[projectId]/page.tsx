...
type ProjectTab = "dashboard" | "tasks" | "members" | "activity" | "settings";
  ...
  if (!matchingOrg || !project || projectError) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Project Access Restricted</h2>
    ...
    <div className="space-y-8 max-w-6xl mx-auto">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400 pt-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-700 dark:text-slate-200 font-medium">{project.owner.name}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="text-slate-400 dark:text-slate-500">Members:</span>
                <span className="text-slate-700 dark:text-slate-200 font-medium">{project.members.length}</span>
              </div>
            </div>
          </div>

          {/* Tab buttons */}
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-950/70 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 self-start md:self-auto overflow-x-auto">
            <button
              onClick={() => setActiveTab("dashboard")}
              className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeTab === "dashboard"
                  ? "bg-white dark:bg-blue-600/20 text-blue-600 dark:text-blue-400 border border-slate-200 dark:border-blue-500/30 shadow-xs"
        ...
        <div className="space-y-6">
          {isStatsLoading || !dashboardStats ? (
            ...
            <>
              {/* Metric Cards Row */}
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-5 shadow-xs">
                ...
                <div
                  className={`rounded-xl border p-5 shadow-xs ${
                    dashboardStats.overdueTasks.count > 0
                      ? "border-red-200 dark:border-red-500/30 bg-red-50 dark:bg-red-950/10 text-red-600 dark:text-red-400"
                ...
                >
                  ...
                  <div
                    className={`text-3xl font-bold ${
                      dashboardStats.overdueTasks.count > 0 ? "text-red-600 dark:text-red-400" : "text-slate-900 dark:text-white"
                    }`}
                  >
              ...
              <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6 shadow-xs">
                <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-3">Status Breakdown</h3>
                <div className="h-3 w-full rounded-full bg-slate-200 dark:bg-slate-800 overflow-hidden flex mb-4">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6 shadow-xs">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 dark:bg-red-950/60 border border-red-200 dark:border-red-800/60 text-red-700 dark:text-red-300 font-semibold">
                      {dashboardStats.overdueTasks.count}
                    </span>
                  </div>
                  ...
                  {dashboardStats.overdueTasks.tasks.length === 0 ? (
                    ...
                    <div className="space-y-2.5">
                      {dashboardStats.overdueTasks.tasks.slice(0, 5).map((t) => (
                        <div
                          key={t.id}
                          className="flex items-center justify-between p-3 rounded-xl border border-red-200 dark:border-red-900/30 bg-red-50 dark:bg-red-950/10"
                        >
                          <div>
                            <div className="text-sm font-medium text-slate-900 dark:text-slate-200">{t.title}</div>
                ...
                <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6 shadow-xs">
                  ...
                  <div className="space-y-3">
                    {dashboardStats.members.map((m) => (
                      <div
                        key={m.id}
                        className="flex items-center justify-between p-2.5 rounded-xl border border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950/40"
                      >
                        <div className="flex items-center gap-3">
        ...
        <div className="space-y-6">
          ...
          {isTasksLoading ? (
            ...
            <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/30 p-12 text-center shadow-xs">
              ...
              <h4 className="text-sm font-semibold text-slate-700 dark:text-slate-300">No tasks found</h4>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-3.5 px-4 max-w-xs sm:max-w-md">
                            <div className="font-medium text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-1">
                              {task.title}
                            </div>
                            {task.description && (
      ...
      {activeTab === "members" && (
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 divide-y divide-slate-200 dark:divide-slate-800/80 overflow-hidden shadow-xs">
            {project.members.map((m) => {
                ...
                <div key={m.id} className="p-4 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{m.user.email}</div>
                    </div>
                  </div>
          ...
          >
            <div className="space-y-4 mt-2">
              <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
      ...
      {activeTab === "settings" && (
        <div className="space-y-8">
          ...
          <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/40 p-6 shadow-xs">
            <h3 className="text-base font-semibold text-slate-900 dark:text-white mb-1">Project Details</h3>
            ...
            <div className="space-y-4 max-w-xl">
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 resize-none"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-950 px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-200 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                  >
          ...
          {isOrgAdmin && (
            <div className="rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50 dark:bg-red-950/10 p-6">
          ...
          >
            <div className="space-y-4 mt-2">
              <div className="p-3.5 rounded-xl bg-red-100 dark:bg-red-950/40 border border-red-200 dark:border-red-800/80 text-xs text-red-800 dark:text-red-300 leading-relaxed">
      ...
      {/* ================= TAB: ACTIVITY ================= */}
      {activeTab === "activity" && matchingOrg && (
        <div className="space-y-4">
          <ActivityFeed organizationId={matchingOrg.id} projectId={projectId} />
        </div>
      )}
