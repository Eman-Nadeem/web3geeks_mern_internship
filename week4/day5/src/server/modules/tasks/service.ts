...
export async function createTask(
  ...
  // Fire-and-forget: real-time + activity
  void Promise.allSettled([
    triggerEvent(Channels.project(projectId), "task:created", dto),
    logActivity({
      organizationId,
      projectId,
      taskId: task.id,
    ...
    ...(input.assigneeId && input.assigneeId !== actorId
      ? [
          createNotification({
            ...
            link: `/dashboard/${organizationId}/projects/${projectId}/tasks/${task.id}`,
            meta: { taskId: task.id },
          }),
        ]
      : []),
...
export async function getMyTasks(
  ...
  // Batch resolve project members & owners to avoid N+1 queries
  const projectIds = Array.from(new Set(tasks.map((t) => t.projectId)));
  const [activeMembers, projects] = await Promise.all([
    prisma.projectMember.findMany({
  ...
  const activeUserKeySet = new Set<string>();
  for (const m of activeMembers) {
    activeUserKeySet.add(`${m.projectId}:${m.userId}`);
  }
  for (const p of projects) {
    activeUserKeySet.add(`${p.id}:${p.ownerId}`);
  }

  const formattedTasks: TaskDTO[] = tasks.map((t) => {
    const isAssigneeActive = !t.assigneeId || activeUserKeySet.has(`${t.projectId}:${t.assigneeId}`);
...
export async function updateTask(
  ...
  // Determine event type for activity log
  const isStatusChanged = input.status !== undefined && input.status !== task.status;
  const isAssigneeChanged = input.assigneeId !== undefined && input.assigneeId !== task.assigneeId;
  const activityType = isOnlyStatusUpdate || isStatusChanged ? "TASK_STATUS_CHANGED" : "TASK_UPDATED";

  void Promise.allSettled([
    triggerEvent(Channels.project(projectId), "task:updated", updatedDto),
    logActivity({
      ...
      meta: {
        ...
        ...(isStatusChanged ? { oldStatus: task.status, newStatus: updated.status } : {}),
      },
    }),
    ...(isAssigneeChanged && updated.assigneeId
      ? [
          logActivity({
            organizationId: updated.project?.organizationId ?? "",
            projectId,
            ...
            actorId,
            type: "TASK_ASSIGNED",
            meta: { taskTitle: updated.title, assigneeId: updated.assigneeId },
          }),
        ]
      : []),
    // Notify new assignee if changed
    ...(input.assigneeId && input.assigneeId !== task.assigneeId && input.assigneeId !== actorId
      ? [
          createNotification({
            userId: input.assigneeId,
            type: "TASK_ASSIGNED",
            ...
            link: `/dashboard/${updated.project?.organizationId ?? ""}/projects/${projectId}/tasks/${taskId}`,
            meta: { taskId },
          }),
        ]
      : []),
...
export async function deleteTask(
  ...
  void Promise.allSettled([
    triggerEvent(Channels.project(projectId), "task:deleted", { taskId, projectId }),
    logActivity({
...
export async function getProjectDashboard(
  ...
  const memberTaskGroups = await prisma.task.groupBy({
    by: ["assigneeId"],
    where: {
      projectId,
      assigneeId: { in: allMembersList.map((m) => m.id) },
    },
    _count: { id: true },
  });

  const memberTaskCountMap = new Map<string, number>();
  for (const group of memberTaskGroups) {
    if (group.assigneeId) {
      memberTaskCountMap.set(group.assigneeId, group._count.id);
    }
  }

  const membersWithCounts = allMembersList.map((m) => ({
    ...m,
    assignedTaskCount: memberTaskCountMap.get(m.id) || 0,
  }));
