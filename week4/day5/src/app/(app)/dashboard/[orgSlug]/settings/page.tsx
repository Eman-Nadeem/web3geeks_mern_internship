  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 backdrop-blur-xl p-6 shadow-sm dark:shadow-xl space-y-6">
          <div className="border-b border-slate-200 dark:border-slate-800/80 pb-4">
            <h2 className="text-base font-semibold text-slate-900 dark:text-white">General Information</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
          ...
          <div className="grid grid-cols-1 gap-6">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`text-[11px] ${
                    descriptionValue.length > 500 ? "text-red-400 font-semibold" : "text-slate-400 dark:text-slate-500"
                  }`}
                >
              ...
              <textarea
                ...
                className="w-full bg-white dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-all disabled:opacity-60 disabled:cursor-not-allowed resize-y"
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950/40">
                <div className="w-16 h-16 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-slate-900 flex items-center justify-center overflow-hidden shrink-0">
                  {logoUrlValue ? (
                    <img
                      src={logoUrlValue}
                      alt="Logo Preview"
        ...
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 backdrop-blur-xl p-6 shadow-sm dark:shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800/80 pb-3">
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-semibold text-slate-900 dark:text-white">Workspace URL Slug</h2>
                {!canChangeSlug && (
          ...
          <div className="space-y-2">
            <div className="relative flex items-center">
              <span className="absolute left-3.5 text-xs text-slate-400 dark:text-slate-500 font-mono">
                /dashboard/
              </span>
              <input
                type="text"
                ...
                placeholder="acme-corp"
                className="w-full pl-28 pr-4 py-2.5 bg-white dark:bg-slate-950/60 border border-slate-300 dark:border-slate-800 rounded-xl text-sm font-mono text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                {...register("slug")}
              />
            </div>
            {errors.slug && (
      ...
      {isOwner && (
        <div className="rounded-2xl border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-950/10 backdrop-blur-xl p-6 shadow-sm dark:shadow-xl space-y-4">
          <div className="border-b border-red-200 dark:border-red-500/20 pb-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-slate-900 dark:text-white">Delete this organization</p>
      ...
      <Dialog isOpen={isSlugConfirmOpen} onClose={() => setIsSlugConfirmOpen(false)}>
        ...
        <div className="space-y-4 py-2">
          <p className="text-sm text-slate-700 dark:text-slate-300">
            Changing the workspace slug to <strong className="font-mono text-slate-900 dark:text-white">/{pendingFormData?.slug}</strong> will immediately invalidate all existing URLs and bookmarks for team members.
          </p>
