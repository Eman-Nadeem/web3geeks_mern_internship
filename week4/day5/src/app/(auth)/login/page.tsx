...
function LoginForm() {
  ...
  const registerLink = initialEmail || redirectPath !== "/dashboard"
    ? `/register?${new URLSearchParams({ ...(initialEmail ? { email: initialEmail } : {}), ...(redirectPath !== "/dashboard" ? { redirect: redirectPath } : {}) }).toString()}`
    : "/register";
  ...
  return (
    <div className="w-full max-w-md space-y-8">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex p-3 rounded-2xl bg-blue-600/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 mb-2">
      ...
      <div className="text-center text-[11px] text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-800/80 rounded-lg p-3 bg-slate-100/80 dark:bg-slate-900/30">
        <p className="font-semibold text-slate-700 dark:text-slate-300 mb-1">Seeded Demo Accounts (dev password: Password123!):</p>
