# Team Collaboration SaaS - Multi-Tenant Platform (Day 3)

A production-grade, secure, multi-tenant "Team Collaboration SaaS" platform built as a single Next.js project. Day 3 extends the core multi-tenant auth (Day 1) and team management/invitations (Day 2) with a full **Project & Task Management System**:
- **Organization-Scoped Projects**: Full CRUD, status roadmap (`PLANNING`, `ACTIVE`, `ON_HOLD`, `COMPLETED`, `ARCHIVED`), and lead ownership.
- **Project Memberships & Access**: Projects drawn strictly from organization members. Pure-function authorization and higher-order middleware preventing any cross-tenant or cross-project data leakage.
- **Task Management & Inline Triage**: Tasks scoped to projects with status (`TODO`, `IN_PROGRESS`, `REVIEW`, `COMPLETED`), priority (`LOW`, `MEDIUM`, `HIGH`, `URGENT`), due dates, and assignments restricted to project members.
- **Task Assignee Retention**: Removing a member preserves their past task assignments with an inactive indicator (`isAssigneeActive: false`). Any project member can reassign tasks.
- **Search & Multi-Filter**: Filter tasks by title, description, status, priority, assignee, due dates, and overdue status with URL synchronization.
- **Cross-Project "My Tasks" View**: Aggregated view across all accessible projects in the workspace.
- **Live Project Dashboard**: Real-time KPI cards, completion %, status distribution bar, overdue tasks alert, and team workload breakdown.
...
### Day 3 API Endpoints (Projects, Tasks & Dashboard)

#### Projects Management
- `GET /api/organizations/:orgId/projects?status=&search=` - List accessible projects (OWNER/ADMIN sees all, MEMBER sees owned or joined).
- `POST /api/organizations/:orgId/projects` - Create project (`{ name, description, status, ownerId }`). (OWNER/ADMIN only).
- `GET /api/organizations/:orgId/projects/:projectId` - Project detail with members and task counts.
- `PATCH /api/organizations/:orgId/projects/:projectId` - Update project attributes or reassign lead. (Project Lead or Org OWNER/ADMIN).
- `DELETE /api/organizations/:orgId/projects/:projectId` - Delete project and cascade tasks/memberships. (Org OWNER/ADMIN only).

#### Project Members
- `GET /api/organizations/:orgId/projects/:projectId/members` - List collaborators on the project.
- `POST /api/organizations/:orgId/projects/:projectId/members` - Add org member to project (`{ userId }`). (Project Lead or Org OWNER/ADMIN).
- `DELETE /api/organizations/:orgId/projects/:projectId/members/:userId` - Remove member from project (tasks retained with `isAssigneeActive: false`).

#### Tasks Management
- `GET /api/organizations/:orgId/projects/:projectId/tasks?search=&status=&priority=&assigneeId=&overdue=&dueBefore=&dueAfter=&page=&limit=` - Multi-filtered tasks.
- `POST /api/organizations/:orgId/projects/:projectId/tasks` - Create task with assignment validation restricted to project members.
- `GET /api/organizations/:orgId/projects/:projectId/tasks/:taskId` - Retrieve task details.
- `PATCH /api/organizations/:orgId/projects/:projectId/tasks/:taskId` - Update task fields, inline status triage, or reassign to project members.
- `DELETE /api/organizations/:orgId/projects/:projectId/tasks/:taskId` - Delete task (Creator, Project Lead, or Org OWNER/ADMIN).

#### Dashboard & Cross-Project Tasks
- `GET /api/organizations/:orgId/projects/:projectId/dashboard` - Project KPIs, completion %, status distribution, overdue tasks, team workload.
- `GET /api/organizations/:orgId/tasks?assigneeId=me&projectId=&status=&priority=&overdue=` - Cross-project workspace tasks.
