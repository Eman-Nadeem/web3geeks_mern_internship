...
export function OrgSubNav({ orgSlug, className }: OrgSubNavProps) {
  ...
  return (
    <nav
      ...
      className={cn(
        "flex items-center gap-1.5 overflow-x-auto p-1 rounded-xl bg-slate-950/60 border border-slate-800/80 backdrop-blur-md",
        className
      )}
    >
      {tabs.map((tab) => {
        ...
        return (
          <Link key={tab.name} href={tab.href}>
            ...
            >
              <Icon className={cn("w-3.5 h-3.5", tab.isActive ? "text-blue-400" : "text-slate-400")} />
              <span>{tab.name}</span>
            </button>
          </Link>
        );
