    ...
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-xl p-6 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-start gap-4">
            <div className="w-14 h-14 rounded-2xl bg-blue-600/20 border border-blue-500/30 text-blue-400 flex items-center justify-center overflow-hidden shrink-0 mt-0.5">
              {orgDetail?.logoUrl ? (
                <img
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Link href={`/dashboard/${orgSlug}/members`} className="block group">
          <Card className="border-slate-800 bg-slate-900/40 hover:border-slate-700 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-none">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400 group-hover:text-blue-400 transition-colors">
                Members
              </CardTitle>
        ...
        <Link href={`/dashboard/${orgSlug}/projects`} className="block group">
          <Card className="border-slate-800 bg-slate-900/40 hover:border-slate-700 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-none">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400 group-hover:text-purple-400 transition-colors">
                Projects
              </CardTitle>
        ...
        <Link href={`/dashboard/${orgSlug}/my-tasks`} className="block group">
          <Card className="border-slate-800 bg-slate-900/40 hover:border-slate-700 transition-colors">
            <CardHeader className="flex flex-row items-center justify-between pb-2 border-none">
              <CardTitle className="text-xs font-semibold uppercase tracking-wider text-slate-400 group-hover:text-emerald-400 transition-colors">
                Tasks
              </CardTitle>
